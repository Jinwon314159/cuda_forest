...
bool garden_truck::load_trees(unsigned int tr_cnt, unsigned int br_cnt, unsigned int ep_cnt)
{
	cudaError_t status;
...
bool garden_truck::load_water(int start_index, int end_index)
{
	...
	for (int i = start_index; i <= end_index; i++)
	{
		...
		sprintf(depth_file_path, ".\\data\\depth\\front\\depth_%d.dat", i);
...
void garden_truck::move_trees_to_warehouse(char* warehouse_address)
{
	...
	fwrite(&branches_count, sizeof(unsigned int), 1, fp);	// count of branches
	fwrite(&endpoints_count, sizeof(unsigned int), 1, fp);	// count of endpoints
...
bool garden_truck::move_trees_from_warehouse(char* warehouse_address, bool do_harvest)
{
	...
	unsigned int _branches_count;
	unsigned int _endpoints_count;
	...
	fread(&_branches_count, sizeof(unsigned int), 1, fp);
	fread(&_endpoints_count, sizeof(unsigned int), 1, fp);
	...
	if (support_version)
	{
		if (strcmp(Version, "V001") == 0)	// V001인 경우 error rate를 읽음
		{
