...
__global__ void absorb(
	...
	fig_fruit *fruits)
{
	//unsigned int nutrient_index = nu_start + threadIdx.x; // blockIdx.x;
	...
	fig_finest_fruit *finest_fruits)
{
	...
	for (int i = 0; i < T; i++)
	{
		...
		{
			if (branches[branch_index].cells[j] == 0 || total_counts == 0)
				continue;
			float prob = (float)branches[branch_index].cells[j] / (float)total_counts;
			probs[j] += prob;
		}
	}
	...
	for (int i = 0; i < MAX_CELL_NUM; i++)
	{
		probs[i] /= T;
#ifndef FINEST_ONLY
		finest_fruits[nutrient_index].probability[i] = probs[i];
#endif
	}
...
#ifdef FINEST_ONLY
	finest_fruits[nutrient_index].probability = (double)max_probability;
...
__global__ void sum_responses(
	...
	fig_twig *twigs, unsigned __int64 tw_start, unsigned __int64 tw_end, unsigned __int64 endpoints_count,
	unsigned short *greenhouse_water)
{
...
bool garden::give(garden_truck *truck)
{
	...
	unsigned __int64 ec_cumulative = 0;
	...
	for (unsigned int t = 0; t < T; t++)
	{
		...
		unsigned __int64 tw_step = nu_step;
		...
		for (unsigned __int64 tw_start = ec_cumulative; tw_start < ec_cumulative + twigs_count; tw_start += tw_step)
		{
			unsigned __int64 tw_end = (tw_start + tw_step < ec_cumulative + twigs_count) ? tw_start + tw_step : ec_cumulative + twigs_count;
...
#ifdef NVIDIA_GeForce_GT_705_620
	unsigned int blocks = 2;
	unsigned int step = tools->prop->maxThreadsPerBlock / 2 * blocks;
#else
	unsigned int blocks = (tools->prop->maxThreadsPerMultiProcessor * tools->prop->multiProcessorCount) / tools->prop->maxThreadsPerBlock;
	unsigned int step = tools->prop->maxThreadsPerBlock * blocks; // tools->prop->maxThreadsPerMultiProcessor;	// truck->nutrients_total_size;
#endif
...
bool garden::replant_from_truck(garden_truck *tk, garden_greenhouse *gh, garden_tools *tl, fig_variety *v, unsigned int t)
{
	...
	truck->ready(T, BUCKET_SIZE);
	greenhouse->ready(T, BUCKET_SIZE);
