...
bool garden_truck::load_trees(unsigned int tr_cnt, unsigned __int64 br_cnt, unsigned __int64 ep_cnt)
{
	...
	if (T != tr_cnt)
	{
		...
		memset(trees, 0, trees_total_size);
	}
	...
	if (branches_count != br_cnt)
	{
		...
		if (status != cudaSuccess) return false;
		memset(branches, 0, branches_total_size);
	}
	...
	if (endpoints_count != ep_cnt)
	{
		...
		if (status != cudaSuccess) return false;
		memset(twigs, 0, twigs_total_size);
	}
...
bool garden_truck::fertilizer_bag(unsigned int nu_cnt)
{
	...
	if (nutrients_count != nu_cnt)
	{
		...
		if (status != cudaSuccess) return false;
		memset(nutrients, 0, nutrients_total_size);
	}
...
bool garden_truck::load_fruits(fig_fruit *fr, unsigned int fr_cnt)
{
	...
	if (fruits_count != fr_cnt)
	{
		...
		if (status != cudaSuccess) return false;
		memset(fruits, 0, fruits_total_size);
	}
...
bool garden_truck::load_water(char* session_path, int start_index, int end_index)
{
	...
	if (buckets_count != end_index - start_index)
	{
		...
		if (status != cudaSuccess) return false;
		memset(water, 0, water_total_amount);
	}
...
#else
	double max_each_err = 0.0;
	for (int t = 0; t < T; t++)
	{
		if (true_count[t] + false_count[t] == 0) continue;
		double each_err = (double)false_count[t] / (double)(true_count[t] + false_count[t]) * 100.0;
		if (each_err > max_each_err)
			max_each_err = each_err;
	}

	error_rate = max_each_err;

	if (max_each_err < best)
	{
		ret = true;
		best = max_each_err;
	}
...
bool garden_truck::load_finest_fruits(fig_finest_fruit *fr, unsigned int fr_cnt)
{
	...
	if (finest_fruits_count != fr_cnt)
	{
		...
		if (status != cudaSuccess) return false;
		memset(finest_fruits, 0, finest_fruits_total_size);
	}
