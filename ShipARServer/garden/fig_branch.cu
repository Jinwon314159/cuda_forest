...
bool fig_branch::dig_up(unsigned __int64 parent_index, unsigned __int64 branch_index, garden_truck *truck, unsigned __int64 *truck_branch_index, unsigned __int64 *truck_endpoint_index)
{
	...
	{
		// 현재 branch의 indexes를 업데이트
		this->left_child_index = ULLONG_MAX;
		this->endpoint_index = *truck_endpoint_index;

		// endpoint라는 건,
		// 1. 아직 선택된 children이 없다는 것
		branch.left_child_index = ULLONG_MAX;
	...
	else
	{
		...
		this->endpoint_index = ULLONG_MAX;

		// branch라는 건,
		// 1. children이 둘 있다는 것
		branch.left_child_index = this->left_child_index;
		// 2. 잎사귀와 잔가지가 사라졌다는 뜻
		branch.endpoint_index = ULLONG_MAX;
...
bool fig_branch::split()
{
	...
	if (this->child[0]) delete this->child[0];
	...
	if (this->child[1]) delete this->child[1];
	...
	cudaFreeHost(this->twigs);
	this->twigs = 0;

	printf("branch_index:%d - splitted\n", this->branch_index);
...
double fig_branch::compute_gain(double prior_entropy, fig_twig* tw)
{
	...
	for (unsigned int i = 0; i < variety->nc; i++)
	{
		if (tw->left_cells[i] + tw->right_cells[i] != 0 && tw->left_total_nutrients + tw->right_total_nutrients != 0)
		{
			double p = (double)(tw->left_cells[i] + tw->right_cells[i]) / (double)(tw->left_total_nutrients + tw->right_total_nutrients);
			prior -= p * log(p) / log(2.0);
		}
	}
	...
	double posterior_entropy = (tw->left_total_nutrients * left_entropy + tw->right_total_nutrients * right_entropy) / (tw->left_total_nutrients + tw->right_total_nutrients); // this->total_nutrients;

	return prior - posterior_entropy;
}