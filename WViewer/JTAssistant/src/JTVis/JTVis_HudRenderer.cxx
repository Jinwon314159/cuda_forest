...
// purpose  :
// =======================================================================
void JTVis_HudRenderer::UpdateItemInfo(const JTData_Item* item, OpenGLFunctions* theGL)
{
	theGL->glUseProgram(0);
	theGL->glBindBuffer(GL_ARRAY_BUFFER, 0);
	theGL->glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, 0);
...

	{
		...
		QString aText;
		aText.append(STR_A3_CONFIRM_DATE);
		aText.append(item->a3Date);
		aText.append(STR_LINE_FEED);

		aText.append(STR_SUPPLY_ARRIVE_RATE);
		aText.append(QString::number(item->supply));
		aText.append(STR_PERCENT);
		aText.append(STR_LINE_FEED);

		aText.append(STR_BOM_CONFIRM_DATE);
		aText.append(item->bomConfirmDate);
		aText.append(STR_LINE_FEED);

		aText.append(STR_BLUEPRINT_FIX_DATE);
		aText.append(item->bomConfirmDate);
		aText.append(STR_LINE_FEED);

		aText.append(STR_WORKER_PER_HOUR);
		aText.append(QString::number(item->workerPerHour));
		aText.append(STR_MAN_HOUR);
		aText.append(STR_LINE_FEED);

		aText.append(STR_DEFAULT_WORKER_PER_HOUR);
		aText.append(QString::number(item->defaultWorkerPerHour));
		aText.append(STR_MAN_HOUR);
		aText.append(STR_LINE_FEED);

		aText.append(STR_CONSTRUCTION_STATUS);
		aText.append(item->constructionStatus);
		aText.append(STR_LINE_FEED);
