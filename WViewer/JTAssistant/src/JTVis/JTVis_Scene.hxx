...
{
  JTVis_Settings (bool  theViewCullingEnabled = false,
				  bool  theSizeCullingEnabled = false,
                  bool  theTrihedronVisible   = false,
				  bool  theStatsOsdVisible = false,
                  bool  theBenchmarkingMode    = false,
                  float theLodQuality         = 1.f,
                  bool  theCameraAnimated     = false,
...
public:
  ...
  void MakeScreenshot (const QString& theScreenshotPath)
  {
  ...
  //! Takes screen shot from current view and saves image in theScreenshotPath.
  void MakeDepthShot(const QString& theScreenshotPath)
  {
      myScreenshotPath = theScreenshotPath;
  ...
  //! Set the export mode
  void SetExportMode(bool theMode, bool theEnableMultiShot);

  //! Set the MVP matrix for export
  void SetExportMVPMatrix(const Eigen::Matrix4f& theMVP);

  //! Set the directory path for exports
  void SetExportPath(const QString& theSession, const QString& theColorPath, const QString& theDepthPath);

  Eigen::Matrix4f GetMVP();
  const JTVis_TargetedCameraPtr GetVisCamera();

  void ImportCamParams(const QString& fileNamePrefix, bool isOrtho);
  void ExportCamParams(const QString& fileNamePrefix);

  void SetMVPMode(bool isEnabled) { myMVPEnabled = isEnabled; }
...
private:
  ...
  //! Write out to PCD File

  void ExportColorDepth(const QString& theColorPath, const QString& theDepthPath);

  void CompareDepth(const QString &filePath, const float *theRenderedDepth);
...
private:
  ...
  JTVis_TargetedCameraPtr myImportedCamera; //!< Current camera object.
  ...
  QOpenGLShaderProgram* myIdShaderProgram;        //!< Shader program that draws object ids into texture.
                                                  //!< Used for selection.
  QOpenGLShaderProgram* myPCShaderProgram;        //!< Shader program that draws object PointCloud coords into texture.
  ...
  QOpenGLFramebufferObject* myColorDepthFbo; //!< OpenGL Frame buffer object.
  ...
  bool setPerformingScreenShot;
  bool isPerformingScreenshot; //!< Indicates viewer need to render into myScreenshotFbo.
  bool isPerformingColorDepthShot; //!< Indicates viewer need to render into myScreenshotFbo.
  ...
  QString myFilename; //!< Opened Filename
  QString mySession; //!< Path for depth data
  QString myColorPath; //!< Path for exporting color files
  QString myDepthPath; //!< Path for depth data

  bool isInExportMode; //!< Is in export mode
  bool myEnableMultiShot;

  Eigen::Matrix4f myExportMVPMatrix; //!< MVP Matrix used for screenshot
  bool myMVPEnabled;
  bool myExportMatrixSet;
  bool myImportedCameraSet;
