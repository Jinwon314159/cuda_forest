...
public:
  ...
  //! Creates new abstract scene graph node.
    JTData_Node() : myIsVisible(Standard_True), myWorkState(-1)
  {
    //
  }
  ...
  //! Set work state of the mesh node
  void SetWorkState(const int theWorkState)
  {
      myWorkState = theWorkState;
  }

  //! Returns work state of the mesh node
  inline int WorkState() const
  {
      return myWorkState;
  }
...
protected:
  ...
  //!< Indicates the work state of the part
  int myWorkState;
