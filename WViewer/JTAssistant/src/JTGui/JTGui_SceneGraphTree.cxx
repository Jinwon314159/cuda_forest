                                                            ...
                                                            const JTData_NodePtr& theNode)
{
  ...
  if (typeid (*theNode.data()) == typeid (JTData_PartNode))
  {
	...
	if (!theNode->Name().isEmpty() && !JTData_JSON::IsEmpty())
	{
		anItem->setCheckState(Qt::Unchecked);

		Tag_Status status = JTData_JSON::TagStatus(theNode->Name());
		if (status != NO_TAG_DATA)
		{
			anItem->setCheckState(Qt::Checked);
			theNode->SetWorkState(((int)status));
		}
	}
  }
