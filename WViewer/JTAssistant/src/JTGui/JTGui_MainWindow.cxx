  ...
  isInExportMode(false)
{
  ...
  connect(ui->actionSaveSelection, SIGNAL(triggered()), this, SLOT(saveSelection()));
  connect(ui->actionUseMVP, SIGNAL(triggered(bool)), this, SLOT(useMVP(bool)));
...
void JTGui_MainWindow::loadSettings()
{
  ...
  bool toShowBrowser = aSettings.value("state/showBrowser", false).toBool();
  bool toShowToolbar = aSettings.value("state/showToolbar", false).toBool();
  ...
  ui->actionEnablePerspective->setChecked (aSettings.value ("state/perspective", true).toBool());
...
#else
  ui->viewCullingCheckBox->setChecked(aSettings.value("settings/view_culling", false).toBool());
  ui->sizeCullingCheckBox->setChecked(aSettings.value("settings/size_culling", false).toBool());

  ui->lodQualitySlider->setValue(aSettings.value("settings/lod_quality", 50).toInt());

  ui->myAnimateCheck->setChecked(aSettings.value("settings/camera_anim", false).toBool());
  ui->myAutoFitCheck->setChecked(aSettings.value("settings/camera_autofit", false).toBool());
  ...
  ui->mySelectionButtons->button(aSettings.value("settings/selection_button", -2).toInt())->setChecked(false);

  if (aSettings.contains("settings/rot_button"))
  {
      ui->myRotationButtons->button(aSettings.value("settings/rot_button", -2).toInt())->setChecked(false);
      ui->myPanningButtons->button(aSettings.value("settings/pan_button", -2).toInt())->setChecked(false);
      ui->myZoomingButtons->button(aSettings.value("settings/zoom_button", -2).toInt())->setChecked(false);

      ui->myZoomWithWheelCheck->setChecked(aSettings.value("settings/zoom_wheel", false).toBool());
#endif
...
// purpose  :
//=======================================================================
void JTGui_MainWindow::loadFile(const QString& theJtFileName, const QString& theBowFileName)
{
    if (theJtFileName.isEmpty())
    return;
  ...
  if (!aGeometrySource->Init(theJtFileName))
  {
  ...
  else
  {
    ...
    if (theBowFileName.isEmpty())
    {
        QString bowFile = theJtFileName;
        bowFile.remove(".jt");
        bowFile += ".json";
        JTData_JSON::LoadTagFile(bowFile);
    }
    else
        JTData_JSON::LoadTagFile(theBowFileName);


    if (JTData_JSON::IsEmpty())
    {
        qInfo("No Tag File found.");
        //exit(0);
    }

    JTData_SceneGraph* aSceneGraph = aGeometrySource->SceneGraph();
    QStack<JTData_Node*> aStack;

    if ((NULL != aSceneGraph) || (!aSceneGraph->Tree().isNull()))
    {
        QList<QString> existTagList;
        ...
        for (;;)
        {
            ...
            if (typeid (*aNode) == typeid (JTData_PartitionNode))
            {
                JTData_PartitionNode* aPartitionNode = static_cast<JTData_PartitionNode*> (aNode);
                for (size_t anIdx = 0; anIdx < aPartitionNode->Children.size(); ++anIdx)
                {
                    aStack.push(aPartitionNode->Children.at(anIdx).data());
                }
            }
            else if (typeid (*aNode) == typeid (JTData_GroupNode))
            {
            ...
            else if (typeid (*aNode) == typeid (JTData_PartNode))
            {
                JTData_PartNode* aPart = static_cast<JTData_PartNode*> (aNode);

                //aPart->SetName(JTData_JSON::CreateShortName(aPart->Name()));

                if (aPart->Children.size() > 0)
                {
                    aPart->Children.at(0)->SetName(aPart->Name());
                }

                if (NO_TAG_DATA == JTData_JSON::TagStatus(aPart->Name()))
                {
                    aPart->SetVisible(false);
                }
                else 
                {
                    existTagList.append(aPart->Name());
                }
            }
        }

        if (0 == existTagList.length())
            exit(0);
    }
    ...
    QFileInfo anInfo(theJtFileName);
  ...
  myLoadedFileName = theJtFileName;
  myRenderWindow->requestActivate();
}
...
//=======================================================================
// function : setOSWindowSize
// purpose  :
//=======================================================================
void JTGui_MainWindow::setOSWindowSize(int width, int height)
{
    ui->myContainer->setFixedSize(QSize(width, height));
}
...
// purpose  :
//=======================================================================
void JTGui_MainWindow::loadOSFile(const QString& theJtFileName, const QString& theBowFileName)
{
    loadFile(theJtFileName, theBowFileName);
}

//=======================================================================
// function : setOSOutputPath
// purpose  :
//=======================================================================
void JTGui_MainWindow::setOSExportPath(const QString& theSession, const QString& theColorPath, const QString& theDepthPath)
{
    if (!myRenderWindow->scene().isNull())
		myRenderWindow->scene()->SetExportPath(theSession, theColorPath, theDepthPath);
    else
        qWarning("A scene hasn't been loaded for the export path to be set");
}
...
// purpose  :
//=======================================================================
void JTGui_MainWindow::setOSMVPMatrix(const Eigen::Matrix4f& theMVP)
{
    if (!myRenderWindow->scene().isNull())
    {
        myRenderWindow->scene()->SetExportMVPMatrix(theMVP);
        ui->actionUseMVP->setChecked(true);
        useMVP(true);
    }
    else
        qWarning("A scene hasn't been loaded for the MVP matrix to be set");
}
...
void JTGui_MainWindow::updateSettings()
{
  ...
  myRenderWindow->scene()->SetMVPMode(ui->actionUseMVP->isChecked());
  myRenderWindow->needToUpdate();
...
//=======================================================================
// function : setOSExportMode
// purpose  :
//=======================================================================
void JTGui_MainWindow::setOSExportMode(bool theMode, bool theEnableMultiShot)
{
    isInExportMode = theMode;

    if (!myRenderWindow->scene().isNull())
    {
        myRenderWindow->scene()->SetExportMode(theMode, theEnableMultiShot);
    }
}

//=======================================================================
// function : makeOSScreenshot
// purpose  :
//=======================================================================
void JTGui_MainWindow::makeOSScreenshot()
{
    if (isInExportMode)
        if (!myLoadedFileName.isEmpty())
        {
            QString aFileName(myLoadedFileName);
            ...
            myRenderWindow->scene()->MakeDepthShot(aFileName);
        }
...
// purpose  :
//=======================================================================
void JTGui_MainWindow::ImportCameraParams(const QString& theFileName, bool isOrtho)
{
    if (!myRenderWindow->scene().isNull())
    {
        myCameraFileName = theFileName;
        myRenderWindow->scene()->ImportCamParams(theFileName, isOrtho);
    }
    else
        qWarning("Scene is not loaded. Camera params cannot be set.");
...
void JTGui_MainWindow::fileLoaded()
{
  ...
  // File Loaded gets called when it finishes loading all the geometry...
  makeOSScreenshot();
  //ImportCameraParams(myCameraFileName);
}
...
// purpose  : save selected item name to json file
//=======================================================================
void JTGui_MainWindow::saveSelection()
{
    QVector<QString> tagArray;

    QTreeWidgetItemIterator it(ui->myTreeWidget);

    while (*it) {
        if (Qt::Checked == (*it)->checkState(0))
        {
            QVariant aData = (*it)->data(0, Qt::UserRole);

            if (aData.data() != NULL)
            {
                JTData_NodePtr aNode = (*it)->data(0, Qt::UserRole).value<JTData_NodePtr>();
                
                if (typeid(*aNode.data()) == typeid (JTData_PartNode))
                    tagArray.push_back(aNode->Name());
            }
        }
    ...
    if (0 < tagArray.length())
    {
        ...
        QString filePath = aSettings.value("DefaultDir").toString() + "/" + myLoadedFileName;
        QString aFileName = QFileDialog::getSaveFileName(this, tr("Save Selection JSON File"),
            filePath, tr("JSON Files (*.json)"));

        JTData_JSON::SaveTagFile(aFileName, tagArray);

        // Place saving camera parameter function here for now...
        aFileName.remove(".json");
        myRenderWindow->scene()->ExportCamParams(aFileName);
...
// purpose  :
//=======================================================================
void JTGui_MainWindow::useMVP(bool isEnabled)
{
    if (myRenderWindow->scene().isNull())
        return;

    myRenderWindow->scene()->SetMVPMode(isEnabled);
}